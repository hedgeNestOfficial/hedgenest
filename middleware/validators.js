...
exports.bankDetailsValidator = (req, res, next) => {
    const schema = joi.object({
        ...
        bankName: joi.string().pattern(/^[A-Za-z\s]+$/) .required() .messages({
           'string.empty': 'Bank name cannot be empty',
           'any.required': 'Bank name is required',
           'string.pattern.base': 'Bank name must contain only letters and spaces'
       }),
       accountName: joi.string().pattern(/^[A-Za-z\s]+$/) .required() .messages({
...
exports.updateValidator = (req, res, next) => {
    const schema = joi.object({
        firstName: joi.string().trim().pattern(/^[A-Za-z]{4,}$/).optional().messages({
            'any.required': 'FirstName is required',
            'string.empty':'FirstName cannot be Empty',
            'string.pattern.base': 'FirstName cannot contain numbers, spaces and must be atleast 4 characters'
		}),
        lastName:joi.string().trim().pattern(/^[A-Za-z]{4,}$/).optional().messages({
            'any.required': 'LastName is required',
            'string.empty':'LastName cannot be Empty',
            'string.pattern.base': 'LastName cannot contain numbers, spaces and must be atleast 4 characters'
		}),
       phoneNumber: joi.string().pattern(/^\d{11}$/).optional().messages({
           'string.empty': 'Phone number cannot be empty',
           'any.required': 'Phone number is required',
           'string.pattern.base': 'Phone number must contain only 11 digits'
       })
